...
async function ensureCommentStatExists(commentId) {
  let stat = await ResourceCommentStat.findByPk(commentId);
  if (!stat) {
    stat = await ResourceCommentStat.create({
...
router.get('/', optionalAuthenticateToken, async (req, res) => {
  try {
    const { resource_id, page = 1, limit = 20 } = req.query;
    if (!resource_id) {
    ...
    const { rows, count } = await ResourceComment.findAndCountAll({
      ...
      include: [
        ...
        { model: Resource, as: 'resource', attributes: ['id', 'title'] },
        { model: ResourceCommentStat, as: 'stats' }
      ]
    });
    ...
    if (req.user && req.user.userId) {
      ...
      if (commentIds.length > 0) {
        const reactions = await ResourceCommentReaction.findAll({
          where: {
    ...
    console.error('获取资源评论列表失败:', error);
...
router.get('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const comment = await ResourceComment.findByPk(id, {
      include: [
        { model: User, as: 'user', attributes: ['id', 'nickname', 'avatar_path', 'role'] },
        { model: Resource, as: 'resource', attributes: ['id', 'title'] },
        { model: ResourceCommentStat, as: 'stats' }
...
router.post('/',
  authenticateToken,
  [
    body('resource_id').isInt().withMessage('resource_id is required'),
  ...
  async (req, res) => {
    try {
      ...
      // 通知资源上传者（如果不是自己评论自己的资源）
      if (resource.uploader_id !== userId) {
        await createNotification({
          user_id: resource.uploader_id,
          type: 'resource',
          title: '你的资源收到新评论',
          content: `你的资源「${resource.title || '未命名资源'}」收到了一条新评论`,
...
router.post('/:id/reactions',
  ...
  async (req, res) => {
    try {
      ...
      const comment = await ResourceComment.findByPk(commentId);
      if (!comment) {
        return res.status(404).json({ status: 'error', message: 'Comment not found' });
      }

      let existing = await ResourceCommentReaction.findOne({ where: { user_id: userId, comment_id: commentId } });
      let stat = await ensureCommentStatExists(commentId);
      const now = new Date();

      if (!existing) {
        await ResourceCommentReaction.create({ user_id: userId, comment_id: commentId, reaction: incoming });
        ...
        if (comment.user_id !== userId && incoming === 'like') {
          await createNotification({
            ...
            type: 'comment',
            title: '你的评论被点赞了',
            content: '你的资源评论收到了一条点赞',
            entity_type: 'resource_comment',
            entity_id: commentId
          });
        }
      ...
      console.error('资源评论点赞/点踩失败:', error);
...
router.get('/:id/stats', async (req, res) => {
  try {
    ...
    console.error('获取资源评论统计失败:', error);
